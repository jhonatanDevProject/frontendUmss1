...
function IndexPresidentes() {
  ...
  const fetchPresidentes = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/presidentesMain');
      const data = response.data.presidentes;
      setPresidentes(data);
    } catch (error) {
  ...
  const handleEdit = (id) => {
    ...
    //history.push(`/admin/EditPresidente//${id}`);
    navigate(`/admin/EditPresidente/${id}`);
  };
...
export default IndexPresidentes;
